...
coordinating **Ops** agent keeps the work moving and tells you what actually needs you. You stop
...
An **Ops** agent coordinates — routing work, spotting connections across desks, and surfacing only
...
the facts, it goes to your **hands-up** queue; everything else they resolve themselves. The Ops
...
> other and Ops tells me where I'm actually needed. I went from babysitting to directing."*
> — pilot user
...
keep memory across sessions, and an Ops agent coordinates — so you're not the relay.
...
Bring the problem, read the Ops's updates, and make the calls the room genuinely can't settle on its
...
shared artifact, each a desk with its own history and frame. We have the shape on disk: an Ops layer,
...
them) and an Ops agent that coordinates **without** the human in the middle. The room is real; the wiring
...
The Ops agent shows you four numbers: hands-up rate (decisions reaching you — too low means resonance,
...
about when to apply them* is encodable is what the product has to prove. The Ops agent is our bet
...
- Is "the room on one artifact" the wedge, or "the Ops agent that coordinates standing desks"? (Two
