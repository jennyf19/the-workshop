...
/// <summary>
/// Win32 Job Object configured with KILL_ON_JOB_CLOSE. The hidden web child is
/// assigned to it so that if the tray process dies abnormally (crash, kill,
/// log-off) the OS tears the child down too — no orphaned headless web server
/// holding the port. On a normal Stop the child is killed first, leaving the
    ...
    public static NativeJob? Create()
    {
        ...
        var info = new JOBOBJECT_EXTENDED_LIMIT_INFORMATION
        {
            BasicLimitInformation = new JOBOBJECT_BASIC_LIMIT_INFORMATION
            {
                ...
                // (desk terminals / pwsh) is NOT pulled into the job, so those
                // are never killed when the job closes — only the web host is.
